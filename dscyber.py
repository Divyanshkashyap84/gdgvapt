...
#                                  /
#
# Author     : kashyap divyansh 
# Tool       : dscyber v1.2
# Usage      : python3 dscyber.py example.com
...
def logo():
    ...
    logo_ascii = """
                     ...
                     Check out our new software, """+bcolors.BG_LOW_TXT+"""NetBot"""+bcolors.ENDC+""" for simulating DDoS attacks - https://github.com/Divyanshkashyap84/gdgvapt
    """
    print(logo_ascii)
    print(bcolors.ENDC)
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("dscyber is updating....Please wait.\n")
    spinner.start()
    ...
    if rs_internet_availability == 0:
        ...
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum dscyber.py | grep .... | cut -c 1-40'
    oldversion_hash = subprocess.check_output(cmd, shell=True)
    oldversion_hash = oldversion_hash.strip()
    os.system('wget -N https://raw.githubusercontent.com/Divyanshkashyap84/gdgvapt/master/dscyber.py -O dscyber.py > /dev/null 2>&1')
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of dscyber." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"dscyber successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
